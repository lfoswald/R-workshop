...
This week's primer will be divided in two parts. Today, we will focus on three aspects:
...
## Arithmetic in `R` `r emo::ji("teacher")`
...
## Logical operators `r emo::ji("thinking_face")`
|       |Operator | 
|:------|:-----|
| Less than | `<` |
...
## Working directory `r emo::ji("house")`
...
However, you should keep in mind that writing out your whole directory path when setting your working directory is NOT a generalizable solution. When others open your R-Script, they won't be able to run it on their machine without manually changing the path. This is bad practice. But there is a better option:
...
# Data Frames `r emo::ji("arrow_down")``r emo::ji("arrow_right")`
...
**Exercise 14**: Print the country that corresponds to the maximum number of conflicts value using the `$` operator and indexing!
```{r, echo = F}
...
## Saving data  `r emo::ji("floppy_disk")`
...
## Dealing with errors in R `r emo::ji("no_entry")`
...
# Best practices `r emo::ji("superhero")`
...
## Project structure `r emo::ji("card_index_dividers")`
